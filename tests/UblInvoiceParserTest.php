 ...
 * tests/fixtures/README.md : à remplacer par les 3 emails réels anonymisés (Piron,
 * OGBAY BV, Proximus) dès que possible. Seule la forme des données reproduit les
...
class UblInvoiceParserTest extends TestCase
{
	...
	protected function pironStyleXml()
	{
		return '<?xml version="1.0" encoding="UTF-8"?>'
			...
			.'<cac:PartyName><cbc:Name>Combustibles Piron</cbc:Name></cac:PartyName>'
	...
	/**
	 * Style "OGBAY BV" : aucun bloc PaymentMeans du tout, oblige le fallback niveau 2
	 * du moteur de matching (voir SPEC.md section 8). Valeurs fictives.
	 */
	protected function ogbayStyleXml()
	{
		return '<?xml version="1.0" encoding="UTF-8"?>'
			.'<Invoice xmlns="'.self::NS_INVOICE.'" xmlns:cbc="'.self::NS_CBC.'" xmlns:cac="'.self::NS_CAC.'">'
			.'<cbc:CustomizationID>'.self::EXPECTED_CUSTOMIZATION_ID.'</cbc:CustomizationID>'
			...
			.'<cac:PartyName><cbc:Name>OGBAY BV</cbc:Name></cac:PartyName>'
	...
	public function testParsesPironStyleInvoice()
	{
	...
	public function testUsesPayableAmountNotTaxInclusiveAmount()
	{
		...
		// PayableAmount.
		$xml = str_replace('<cbc:PayableAmount>121.00</cbc:PayableAmount>', '<cbc:PayableAmount>21.00</cbc:PayableAmount>', $this->pironStyleXml());
	...
	{
		$parser = new UblInvoiceParser();
		$data = $parser->parse($this->ogbayStyleXml());
	...
	public function testNormalizePaymentRefKeepsOnlyDigits()
	{
		...
		// Style "Proximus" : communication présente mais sans séparateurs (voir SPEC.md
	...
	{
		$xml = str_replace(self::EXPECTED_CUSTOMIZATION_ID, 'urn:something:else', $this->pironStyleXml());
